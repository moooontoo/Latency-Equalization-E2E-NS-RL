...
def rlRan(sla,target_net,act_net,max_episodes = 5000):
    ...
    for i_episode in range(episodes):
        ...
        # compute reward
        r, se, qoe, ep_latency = net.compute_reward(a_p, a_c, sla)
        ep_rewards.append(np.sum(r))
        ep_se.append(np.sum(se))
        ep_qoe.append(np.sum(qoe))
        memory.add(s, r, a_p)
        s = s_  # 状态转移
        ...
        if (i_episode + 1) > 200:  # 原2000，原200，还是200比较合适
            ...
            for i in range(32):
                x[i][0][0] = torch.max(x1_c[i][0])
                x[i][0][1] = torch.max(x1_p[i][0])


                x[i][0][2] = torch.max(x2_c[i][0])
                x[i][0][3] = torch.max(x2_p[i][0])


                x[i][0][4] = torch.max(x3_c[i][0])
                x[i][0][5] = torch.max(x3_p[i][0])


                x[i][0][6] = torch.max(x4_c[i][0])
                x[i][0][7] = torch.max(x4_p[i][0])


                x[i][0][8] = torch.max(x5_c[i][0])
                x[i][0][9] = torch.max(x5_p[i][0])


                x[i][0][10] = torch.max(x6_c[i][0])
                x[i][0][11] = torch.max(x6_p[i][0])


                x[i][0][12] = torch.max(x7_c[i][0])
                x[i][0][13] = torch.max(x7_p[i][0])


                x[i][0][14] = torch.max(x8_c[i][0])
                x[i][0][15] = torch.max(x8_p[i][0])


                x[i][0][16] = torch.max(x9_c[i][0])
                x[i][0][17] = torch.max(x9_p[i][0])


                x[i][0][18] = torch.max(x10_c[i][0])
                x[i][0][19] = torch.max(x10_p[i][0])


                x[i][0][20] = torch.max(x11_c[i][0])
                x[i][0][21] = torch.max(x11_p[i][0])


                x[i][0][22] = torch.max(x12_c[i][0])
                x[i][0][23] = torch.max(x12_p[i][0])


                x[i][0][24] = torch.max(x13_c[i][0])
                x[i][0][25] = torch.max(x13_p[i][0])


                x[i][0][26] = torch.max(x14_c[i][0])
                x[i][0][27] = torch.max(x14_p[i][0])


                x[i][0][28] = torch.max(x15_c[i][0])
                x[i][0][29] = torch.max(x15_p[i][0])


                x[i][0][30] = torch.max(x16_c[i][0])
                x[i][0][31] = torch.max(x16_p[i][0])


                x[i][0][32] = torch.max(x17_c[i][0])
                x[i][0][33] = torch.max(x17_p[i][0])


                x[i][0][34] = torch.max(x18_c[i][0])
                x[i][0][35] = torch.max(x18_p[i][0])


                x[i][0][36] = torch.max(x19_c[i][0])
                x[i][0][37] = torch.max(x19_p[i][0])


                x[i][0][38] = torch.max(x20_c[i][0])
                x[i][0][39] = torch.max(x20_p[i][0])
            ...
            if (i_episode + 1) % 100 == 0:  # 原来是2000
                ...
                print('step:', (i_episode + 1), 'avg_ep_reward:', avg_ep_reward)
                # with open('ep_rewards.csv', 'ab') as f:
                #     np.savetxt(f, np.array([(i_episode + 1), avg_ep_reward]), delimiter=',', fmt='%.4f')

                # if max_avg_ep_reward * 0.9 <= avg_ep_reward:  # 如果目前reward大于之前reward的百分之90
                #     #     # self.step_assign_op.eval({self.step_input: self.step + 1}) #m，感觉像是记录一共多少次记录
                #     # save()
                #     # self.memory.save()
                #     max_avg_ep_reward = np.max([max_avg_ep_reward, avg_ep_reward])
                # save()
                ep_rewards = []
                se = []
                qoe = []

    torch.save(act_net.state_dict(), '../models/actNature')
    torch.save(target_net.state_dict(), '../models/targetNature')
    return ep_latency, s
if __name__ == '__main__':
    sla = [0.8]*20
    target_net, act_net = Net(), Net()
    l, s = rlRan(sla, target_net, act_net, 5000)
